...
const Team: React.FC = () => {
  return (
    <div className="pt-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
            {pillars.map((item, idx) => (
              <FadeIn key={item.label} delay={idx * 0.1}>
                <div className="flex flex-col items-center text-center p-8 rounded-2xl bg-punchline-light hover:bg-blue-50 transition-colors">
                  <div className="w-12 h-12 bg-punchline-blue text-white rounded-xl flex items-center justify-center mb-4">
                    {item.icon}
                  </div>
                  <h3 className="font-bold text-punchline-black text-lg mb-2">{item.label}</h3>
                  <p className="text-punchline-gray text-sm">{item.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-8">
            {teamMembers.map((member, idx) => (
              <FadeIn key={member.name} delay={idx * 0.12}>
                <motion.article
                  whileHover={{ y: -8, boxShadow: '0 24px 60px rgba(11,86,198,0.14)' }}
                ...
                >
                  ...
                  <div className="relative inline-block mb-6">
                    <div
                      className={`w-24 h-24 rounded-full bg-gradient-to-br ${member.gradient} flex items-center justify-center text-white font-black text-xl shadow-lg mx-auto group-hover:scale-105 transition-transform duration-300`}
                      role="img"
                  ...
                  <p className="text-punchline-gray text-sm leading-relaxed">{member.description}</p>
                </motion.article>
              </FadeIn>
            ))}
          </div>
      ...
      {/* CTA */}
      <section className="py-24 bg-punchline-black text-white" aria-label="Call to action">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <FadeIn>
            ...
            >
              Get in Touch <ChevronRight className="ml-2 w-5 h-5" aria-hidden="true" />
            </Link>
          </FadeIn>
        </div>
      </section>
